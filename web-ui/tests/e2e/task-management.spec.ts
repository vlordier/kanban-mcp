...
test.describe('Task Management UX', () => {
  ...
  test('should delete task with confirmation', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Are you sure you want to delete "Task to Delete"')
    ).toBeVisible();
  ...
  test('should support drag and drop between columns', async ({ page }) => {
    ...
    const taskCard = page
      .locator('div:has(h3:has-text("To Do"))')
      .locator('div:has-text("Draggable Task")')
      .first();
    const inProgressColumn = page.locator('div:has(h3:has-text("In Progress"))');
    ...
    const inProgressCount = page
      .locator('div:has(h3:has-text("In Progress"))')
      .locator('span')
      .first();
