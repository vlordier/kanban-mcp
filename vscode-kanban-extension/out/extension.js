...
const EmbeddedServer_1 = require("./server/EmbeddedServer");
...
async function activate(context) {
    ...
    embeddedServer = new EmbeddedServer_1.EmbeddedServer();
    let serverPort;
    try {
    ...
    const errorHandler = ErrorHandler_1.ErrorHandler.getInstance(logger);
    ...
    const webviewProvider = new KanbanWebviewProvider_1.KanbanWebviewProvider(context, mcpClient, embeddedServer);
    // Register commands
    ...
    const openBoardToColumnCommand = vscode.commands.registerCommand('kanban-mcp.openBoardToColumn', (boardId, columnId) => {
        webviewProvider.showKanbanBoard(boardId, columnId);
    });
    const createTaskFromFileCommand = vscode.commands.registerCommand('kanban-mcp.createTaskFromFile', async (fileUri) => {
        if (!fileUri) {
        ...
        try {
            ...
            if (boards.length > 1) {
                const boardChoice = await vscode.window.showQuickPick(boards.map(board => ({ label: board.name, description: board.goal, board })), { placeHolder: 'Select board for the new task' });
                if (!boardChoice) {
                    return;
                }
            ...
            const columnChoice = await vscode.window.showQuickPick(boardDetails.columns.map(col => ({
                label: col.name,
    ...
    const deleteBoardCommand = vscode.commands.registerCommand('kanban-mcp.deleteBoard', async (treeItem) => {
    ...
    const showHealthStatusCommand = vscode.commands.registerCommand('kanban-mcp.showHealthStatus', async () => {
        ...
        if (isHealthy) {
            vscode.window.showInformationMessage(`✅ Kanban Server is healthy\nConnected to: ${serverUrl}`, 'Test Connection').then(selection => {
            ...
            vscode.window.showErrorMessage(`❌ Kanban Server is unreachable\nServer URL: ${serverUrl}\n\nPlease check if the server is running.`, 'Retry Connection', 'View Logs').then(selection => {
    ...
    async function updateHealthStatus(client) {
    ...
    context.subscriptions.push(showKanbanBoardCommand, createBoardCommand, refreshBoardsCommand, openBoardCommand, openBoardToColumnCommand, createTaskFromFileCommand, quickCreateTaskCommand, deleteBoardCommand, showHealthStatusCommand, healthCheckDisposable);
    // Auto-refresh setup
    const config = vscode.workspace.getConfiguration('kanban-mcp');
