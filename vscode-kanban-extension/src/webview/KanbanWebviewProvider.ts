...
export class KanbanWebviewProvider {
    ...
    public showKanbanBoard(boardId?: string, columnId?: string): void {
        ...
        const panel = vscode.window.createWebviewPanel(
            'kanbanBoard',
            boardId ? `Kanban Board - ${boardId}` : 'Kanban Board',
                            ...
                            switch (method) {
                                ...
                                case 'POST':
                                    if (endpoint === '/boards') {
                                        ...
                                        const serverData = {
                                            boards: requestData.boards.map((board: any) => ({
                                                id: board.id,
                                                name: board.name,
                                                goal: board.goal,
                                            ...
                                            columns: requestData.columns.map((column: any) => ({
                                                id: column.id,
                                                name: column.name,
                                                position: column.position,
                                            ...
                                            tasks: requestData.tasks.map((task: any) => ({
                                                id: task.id,
                                                title: task.title,
                                                content: task.content || '',
                                                position: task.position,
    ...
    private getWebviewContent(webview: vscode.Webview, boardId?: string, columnId?: string): string {
    ...
    private getFallbackContent(webview: vscode.Webview, boardId?: string): string {
...
<body>
    ...
    private async handleFileSystemExport(requestData: any): Promise<any> {
        try {
            ...
            const result = await vscode.window.showInformationMessage(
                `Kanban data exported successfully to:\n${exportDir}`,
                'Open Folder',
                'OK'
            );
